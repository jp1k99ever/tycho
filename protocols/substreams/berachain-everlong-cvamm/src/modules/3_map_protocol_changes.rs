...
pub fn map_protocol_changes(
    ...
    for tx_components in new_components.tx_components.iter() {
        ...
        for component in tx_components.components.iter() {
            ...
            builder.add_entity_change(&cvamm::indexed::initial_entity_change(&component.id, pool));
        }
    }
    ...
    for tx in block.transactions() {
        for (log, _) in tx.logs_with_calls() {
            ...
            let Some(event) = cvamm::events::decode_cvamm_log(log) else {
                continue;
            };
            ...
            if let Some(entity_change) =
                cvamm::indexed::entity_change_for_event(&component_id, pool, &event)
            {
                builder.add_entity_change(&entity_change);
            }
            for balance_change in cvamm::indexed::balance_changes_for_event(
                &component_id,
                &pool.stable,
                &pool.volatile,
                &event,
...
fn pool_by_component_id<'a>(config: &'a Config, component_id: &str) -> Option<&'a AlmConfig> {
    config
        .pools
        .iter()
...
fn pool_by_address<'a>(config: &'a Config, address: &[u8]) -> Option<&'a AlmConfig> {
    config
        .pools
        .iter()
        .find(|pool| address == pool.alm)
}
