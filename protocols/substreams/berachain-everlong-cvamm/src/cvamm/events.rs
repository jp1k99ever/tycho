...
pub enum CvammEvent {
    ...
    SwapFeeCharged { fee0_nonzero: bool, fee1_nonzero: bool, fee_wad: Vec<u8> },
    /// Reserve totals INCLUDING idle — used for component balances (TVL), not for
    /// the solvency-clamp reserve attributes.
    Sync { reserve0: Vec<u8>, reserve1: Vec<u8> },
    Recentered { anchor_after: Vec<u8>, kappa_after: Vec<u8>, x_after: Vec<u8> },
    CurveRetuned { a_wad: Vec<u8>, span_up_wad: Vec<u8>, span_dn_wad: Vec<u8> },
    IdleDeployed { kappa_after: Vec<u8> },
    FeesCompounded { kappa_after: Vec<u8> },
    ReservesRetracted,
...
pub fn event_attributes(event: &CvammEvent) -> Vec<tycho::Attribute> {
    match event {
        ...
        CvammEvent::ReservesRetracted => vec![
            attribute(attrs::RETRACTED, vec![1u8]),
            attribute(attrs::KAPPA, vec![0u8]),
        ],
...
fn be_bytes(value: &substreams::scalar::BigInt) -> Vec<u8> {
    ...
    debug_assert!(
        ...
        "unsigned ABI value decoded negative"
    );
    bytes
}
