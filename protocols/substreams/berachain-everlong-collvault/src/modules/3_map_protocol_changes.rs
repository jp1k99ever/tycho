...
pub fn map_protocol_changes(
    ...
    for tx_components in new_components.tx_components.iter() {
        ...
        for component in tx_components.components.iter() {
            ...
            builder.add_entity_change(&collvault::indexed::initial_entity_change(
                &component.id,
                venue,
            ));
        }
        ...
        for tx in block.transactions() {
            for (log, _) in tx.logs_with_calls() {
                let event = if log.address == venue.rebalancer {
                    collvault::events::decode_rebalancer_log(log)
                } else if log.address == venue.alm {
                    collvault::events::decode_alm_log(log)
                } else {
                    None
                };
                ...
                if let Some(entity_change) =
                    collvault::indexed::entity_change_for_event(&component_id, &event)
                {
                    builder.add_entity_change(&entity_change);
                }
                for balance_change in collvault::indexed::balance_changes_for_event(
                    &component_id,
                    &venue.stable,
                    &venue.volatile,
                    &event,
